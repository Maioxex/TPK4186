...
class Optimizer:
	...
	def runExperiment(self, arrivalTimes, numberOfExecutions):
		...
		for _ in range(numberOfExecutions):
			simulator.scheduleCustomerArrivals(arrivalTimes)
			successful = simulator.simulationLoop()
			if successful:
...
def createArrivalTimes(numberOfCustomers, timeBetweenCustomers):
	return [i*timeBetweenCustomers for i in range(numberOfCustomers)]


queueCapacity = 3
simulator = Simulator(queueCapacity)

arrivalTimes = createArrivalTimes(10, 2)
print(arrivalTimes)
simulator.scheduleCustomerArrivals(arrivalTimes)
